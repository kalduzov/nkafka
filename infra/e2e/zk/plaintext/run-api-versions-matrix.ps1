...
function Invoke-ApiVersionsTestRun {
    ...
    try {
        ...
        [Environment]::SetEnvironmentVariable("NKAFKA_E2E_TOPOLOGY_MODE", "zk", "Process")
...
foreach ($kafkaVersion in $KafkaVersions) {
    ...
    $projectName = "nkafka-e2e-zk-plaintext-$versionToken"
    $containerName = "nkafka-e2e-zk-plaintext-$versionToken"
    $zookeeperContainerName = "nkafka-e2e-zk-plaintext-zookeeper-$versionToken"
    ...
    Write-Host ""
    Write-Host "=== Kafka $kafkaVersion / ZooKeeper plaintext / ApiVersions E2E ===" -ForegroundColor Cyan
