...
| Producer | [src/NKafka/Clients/Producer/Internals/TransactionManager.cs](K:\nkafka\src\NKafka\Clients\Producer\Internals\TransactionManager.cs) | `SendOffsetsToTransactionAsync()` пока `Task.CompletedTask` | Нет transactional offsets flow | P0 | Реализовать AddOffsetsToTxn / TxnOffsetCommit path и покрыть тестами |
...
| Consumer | [src/NKafka/Clients/Consumer/Internal](K:\nkafka\src\NKafka\Clients\Consumer\Internal) | KIP-848 message support уже есть, но основной runtime path всё ещё classic-oriented | Новый consumer protocol пока не доведён до runtime feature level | P1 | Отдельно выделить task на KIP-848 runtime integration |
| Admin | [src/NKafka/Clients/Admin/AdminClient.cs](K:\nkafka\src\NKafka\Clients\Admin\AdminClient.cs) | `DescribeAclsAsync()` возвращает пустой result | ACL admin API misleading | P0 | Реализовать request/response mapping для DescribeAcls |
| Admin | [src/NKafka/Clients/Admin/AdminClient.cs](K:\nkafka\src\NKafka\Clients\Admin\AdminClient.cs) | `CreateAclsAsync()` возвращает пустой result | ACL create фактически отсутствует | P0 | Реализовать CreateAcls flow и tests |
| Admin | [src/NKafka/Clients/Admin/AdminClient.cs](K:\nkafka\src\NKafka\Clients\Admin\AdminClient.cs) | `DeleteAclsAsync()` возвращает пустой result | ACL delete фактически отсутствует | P0 | Реализовать DeleteAcls flow и tests |
| Admin | [src/NKafka/Clients/Admin/AdminClient.cs](K:\nkafka\src\NKafka\Clients\Admin\AdminClient.cs) | `DescribeConfigsAsync()` возвращает пустой result | Config admin API не готов | P1 | Реализовать DescribeConfigs request/response path |
| Admin | [src/NKafka/Clients/Admin/AdminClient.cs](K:\nkafka\src\NKafka\Clients\Admin\AdminClient.cs) | `AlterConfigsAsync()` возвращает пустой result | Legacy config mutation API не реализован | P1 | Реализовать basic flow или явно пометить unsupported |
| Admin | [src/NKafka/Clients/Admin/AdminClient.cs](K:\nkafka\src\NKafka\Clients\Admin\AdminClient.cs) | `IncrementalAlterConfigsAsync()` возвращает пустой result | Современный config mutation API не реализован | P1 | Реализовать incremental alter configs flow |
