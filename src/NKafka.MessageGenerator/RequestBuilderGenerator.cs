...
public class RequestBuilderGenerator
{
    ...
    public RequestBuilderGenerator(string ns)
    {
    ...
    private void GenerateResponseBuilder(IEnumerable<(string ApiKeyName, short ApiKey, string ClassName)> responseInformation)
    {
        ...
        _codeGenerator.AppendLine("internal static partial class RequestBuilder");
        ...
        _codeGenerator.AppendLine("public static IRequestMessage Build(ApiKeys apiKey, ApiVersion apiVersion, byte[] span)");
        _codeGenerator.AppendLeftBrace();
        _codeGenerator.IncrementIndent();
