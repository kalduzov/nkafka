...
public sealed partial class DeleteTopicsRequestMessage
{
    ...
    {
        var request = new DeleteTopicsRequestMessage
        {
            TimeoutMs = timeoutMs
        };

        if (apiVersion <= ApiVersion.Version5)
        {
            request.TopicNames.AddRange(topicsName);
        }
        else
        {
            request.Topics = topicsName.Select(tn => new DeleteTopicStateMessage
                {
                    Name = tn
                })
                .ToList();
