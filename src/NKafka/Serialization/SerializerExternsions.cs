...
public static class SerializationExtensions
{
    ...
    /// <returns></returns>
    public static byte[] ToKafkaBytes<T>(this T? value)
    {
        if (value is null)
        {
            return Serializers.Null.Serialize(default);
        }

        var type = typeof(T);

        return _defaultSerializers.TryGetValue(type, out var serializer)
            ? ((ISerializer<T>)serializer).Serialize(value)
            : throw new NotSupportedException($"Serialization of type {type} is not supported");
    }
}