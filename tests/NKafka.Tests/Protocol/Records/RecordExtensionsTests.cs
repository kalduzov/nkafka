...
public class RecordExtensionsTests
{
    ...
    public void WriteToTest()
    {
        ...
        var arrayBuffer = new ArrayBuffer(true, false, _validRecord.Length);
        var bufferWriter = new BufferWriter(ref arrayBuffer);
        record.WriteTo(ref bufferWriter);

        arrayBuffer.DangerousGetFirstBuffer().Should().BeEquivalentTo(_validRecord);
    }
}