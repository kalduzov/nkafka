...
public class IntegerSerializerTests
{
    ...
    public async Task SerializeAsync_Successful(int value, string serializeValueAsByteString)
    {
        var serializeValue = Convert.FromHexString(serializeValueAsByteString);
        var serializer = new IntegerSerializer();
    ...
    [InlineData(int.MaxValue, "7FFFFFFF")]
    [InlineData(int.MinValue, "80000000")]
    public void Serialize_Successful(int value, string serializeValueAsByteString)
    {
        var serializeValue = Convert.FromHexString(serializeValueAsByteString);
        var serializer = new IntegerSerializer();
